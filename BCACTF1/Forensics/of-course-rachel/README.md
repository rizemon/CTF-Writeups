...
In ```snapshot.zip``` were 5 .png files, each with contents looking like this. However, it would be very troublesome to slowly convert these to text. Hence, I went to search for any Image to Text websites.

![](./2.png)

I came across ```https://www.onlineocr.net/```, which is able to accept images and documents and convert them into text using Optical Character Recognition. Seeing that the acronym is "OCR", I immediately knew that I was on the right track based on the challenge's name (If you looked at the first letter of each word, you will get "OCR")

![](./3.png)

Using Cyberchef and the "From Hex" operation, we observed that the pictures contained some python code. If we append all of them together,
...
Running this prints out the flag. Note that this script needs to run using python3.
